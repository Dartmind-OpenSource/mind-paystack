...
Get the full **mobile-ready** experience with UI components:

```bash
# Add both core SDK and Flutter integration
...
|---------|----------|----------|
| `mind_paystack` | Core SDK for any Dart project | API clients, models, services |
| `mind_paystack_flutter` | Flutter-specific features | UI widgets, platform integration |
...
Future<bool> verifyPayment(String reference) async {
  ...
  try {
    ...
    if (verification.data.status == TransactionStatus.success) {
      print('Payment successful! ₦${verification.data.amount / 100}');
...
class PaymentService {
  ...
  static Future<bool> verifyPayment(String reference) async {
    ...
    try {
      ...
      return result.data.status == TransactionStatus.success;
    } on MindException catch (e) {
      print('Verification failed: ${e.message}');
      return false;
    }
...
🎨 **Flutter Integration:** [Flutter Widgets & Components →](./flutter-integration)  
⚙️ **Advanced Configuration:** [SDK Configuration Options →](./configuration)  
🔧 **Error Handling:** [Exception Management →](./error-handling)  
📊 **Testing:** [Unit Testing Your Integration →](./testing)  
